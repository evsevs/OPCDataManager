...
STDMETHODIMP CCXMLPlug::GetTags(ITEMID *ClientTagID, DWORD dwCount, OPCITEMSTATE **pOutStruct, HRESULT **ppErrors)
{
	...
	m_LogFile<<EN.c_str();
//	m_LogFile<<"Непосредственно перед созданием потока";
	CAsyncXMLRead AR(EN.c_str());
//	m_LogFile<<"Непосредственно перед запуском потока";
	AR.run();
	Sleep(10);
//	m_LogFile<<"Непосредственно после запуска потока";
	//m_LogFile<<"!";
	...
	_variant_t outVal((bool)FALSE);
	try{
//		m_LogFile<<"DEBUG: Перед Createinstance!";
	//	hr=m_Doc.CreateInstance("Msxml2.DOMDocument.3.0");//("msxml2.domdocument");
//		m_LogFile<<"Непосредственно после Createinstance";
...
//	outVal=xxxDD->load(newpath.c_str());
	AR.m_url=newpath.c_str();
//	m_LogFile<<"DEBUG: Перед LoadNew!";
	AR.LoadNew();
//	m_LogFile<<"DEBUG: После LoadNew!";
	//m_LogFile<<"LoadNew";
...
//	m_LogFile<<"DEBUG: Перед OpenEvent!";
	HANDLE MyEvent = OpenEvent(EVENT_ALL_ACCESS,FALSE,AR.m_EventName.c_str());
//	m_LogFile<<"DEBUG: После OpenEvent!";
	if (WaitForSingleObject(MyEvent,AsyncTimeout)==WAIT_OBJECT_0) //GOOD!!!
	{
		//m_LogFile<<"before Loaded GOOD!!!";
		//m_LogFile<<AR.m_loadedXML;
		_bstr_t bbb(AR.m_loadedXML,true);
		
		m_Doc->loadXML(bbb);
	//	m_LogFile<<"Loaded GOOD!!!";
	...
	CloseHandle(MyEvent);
//	m_LogFile<<"WARNING: После Close Handle";
	AR.Stop();
//	m_LogFile<<"WARNING: После AR.Stop()";
	//while (!XMLReady)//AsyncResult!=(long)4)
	//{   AR.getStatus();
	//	Sleep(100);
	//	AsyncResult=AR.m_CurrentState;
	//	xCount+=100;
...
//===================================================================================
	...
	}catch(...)
	{
		//ATLTRACE("ERROR1 %s\n",m_XMLPath.c_str());
	//	return E_FAIL;
		m_LogFile<<"ERROR: Неопознанная ошибка.";
		isNormalLoaded=false;
	}
	...
	*ppErrors=(HRESULT*)CoTaskMemAlloc(sizeof(HRESULT)*dwCount);
	...
	for(int i=0;i<dwCount;i++)
	{
		...
		if ((ml->length>0l)&&(isNormal))
		{
			...
			std::string vvv= this->ChangeFloatString(/*(char*)sss*/(char*)ml->item[0l]->text);
			sss = (char *)vvv.c_str();
